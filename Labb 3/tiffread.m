function [r,g,b,x] = tiffread(filename)
...
%
%	[C,M,Y,K] = TIFFREAD('filename') reads the TIFF file 'filename'
%	and returns the CMYK image in the matrices C,M,Y,K. 
%
...
%       Modified by Jorgen Rydenius to handle 48-bit RGB images and CMYK.
%       The fourth output argument is used when a CMYK image is to be read.
%
%	Ability to read CIELab is also added. In this case the first
%	argument L is between 0 and 100. The other two, a* and b*, are
%	between -127 and 127. 
...
for i = 1:num_entries
  ...
  elseif (tag == 258)
    ...
    if (bits(1) ~= 1) & (bits(1) ~= 4) & (bits(1) ~= 8) & (bits(1) ~= 16)
      error('Unsupported number of bits per sample.');
    end
    num_samples = count;
    if nargout <= 1,
      if num_samples==3, % RGB image or CIELab
        if bits(1)==8
          r = 24;
        else
          r = 48;
        end
      elseif num_samples==4, % CMYK image
        r = 32;
    ...
    if (num_samples == 4) & (nargout ~= 4),
      disp('Warning: File contains a CMYK image. ')
      disp('   Use [c,m,y,k] = tiffread(''filename'') instead.')
    end
...
if (num_samples == 3) % If this is an RGB image or CIELab
  % rgb color
  if  (photo_type == 8) % CIELab image
    r = draw(:,1:3:width*3)/2.55; % L component
    a = find(r < 0);
    r(a) = r(a) + 100;
    g = draw(:,2:3:width*3); % a component
    b = draw(:,3:3:width*3); % b component
    a = find(b > 0.5);
    b(a) = b(a)-1;
  elseif (bits(1) == 8) % 24-bit RGB image
    r = draw(:,1:3:width*3)/255;
    g = draw(:,2:3:width*3)/255;
    b = draw(:,3:3:width*3)/255;
  else % 48-bit rgb image
    r = (256*draw(:,1:6:width*6)+draw(:,2:6:width*6))/65535;
    g = (256*draw(:,3:6:width*6)+draw(:,4:6:width*6))/65535;
    b = (256*draw(:,5:6:width*6)+draw(:,6:6:width*6))/65535;
  end
  if (nargout == 2) % Convert to an indexed image if user requests it.
    [r,g] = rgb2ind(r,g,b,.2);
  end
elseif (num_samples == 4) % If this is a CMYK image
  r = draw(:,1:4:width*4)/255;   % C component
  g = draw(:,2:4:width*4)/255;   % M component
  b = draw(:,3:4:width*4)/255;   % Y component
  x = draw(:,4:4:width*4)/255;   % K component
else
